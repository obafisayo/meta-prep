...
export async function getStudyState() {
  await ensureSchema();
  const rows = await sql`SELECT weak_areas, sessions FROM study_state WHERE id = 1`;
  if (!rows.length) return { weakAreas: [], sessions: 0 };
  return { weakAreas: rows[0].weak_areas || [], sessions: rows[0].sessions || 0 };
}

export async function saveStudyState(weakAreas, sessions) {
  await ensureSchema();
  await sql`
    INSERT INTO study_state (id, weak_areas, sessions)
    VALUES (1, ${JSON.stringify(weakAreas)}::jsonb, ${sessions})
    ON CONFLICT (id) DO UPDATE SET weak_areas = EXCLUDED.weak_areas, sessions = EXCLUDED.sessions
  `;
}
